...
MMISCENE_API MMIStandard::MVector3 ToMVec3(const FVector& fVec, double multiplier = 0.01);

//Convert Unreal vector to MOSIM std::vector<double>
//By default, multiplier is set to 0.01 (cm -> m, cm/s^2 -> m/s^2) for coordinates, lengths, forces,
//but can take other values for other units conversion.
//Should be 1 for dimensionless units i.e. scale or angular velocity.
MMISCENE_API std::vector<double> ToDVec(const FVector& fVec, double multiplier = 0.01);
...
//but can take other values for other units conversion.
//Should be 1 for dimensionless units i.e. scale or angular velocity.
MMISCENE_API FVector ToFVec3(const MMIStandard::MVector3& mVec, double multiplier = 100);

//Convert MOSIM vector to Unreal vector
//By default, multiplier is set to 100 (m -> cm, m/s^2 -> cm/s^2) for coordinates, lengths, forces,
//but can take other values for other units conversion.
//Should be 1 for dimensionless units i.e. scale or angular velocity.
MMISCENE_API FVector ToFVec3(const std::vector<double>& dVec, double multiplier = 100);

MMISCENE_API MMIStandard::MQuaternion ToMQuat(const FQuat& fquat);

MMISCENE_API FQuat ToFQuat(const MMIStandard::MQuaternion& mquat);

//Convert vector elements to Unreal 2D vector (XY plane) starting from specified index
//By default, multiplier is set to 100 (m -> cm, m/s^2 -> cm/s^2) for coordinates, lengths, forces,
//but can take other values for other units conversion.
//Should be 1 for dimensionless units i.e. scale or angular velocity.
MMISCENE_API FVector ToFVec2(const std::vector<double>& dVec, int startIndex, double multiplier = 100);

//Convert vector elements to Unreal 3D vector starting from specified index
//By default, multiplier is set to 100 (m -> cm, m/s^2 -> cm/s^2) for coordinates, lengths, forces,
//but can take other values for other units conversion.
//Should be 1 for dimensionless units i.e. scale or angular velocity.
MMISCENE_API FVector ToFVec3(const std::vector<double>& dVec, int startIndex, double multiplier = 100);
