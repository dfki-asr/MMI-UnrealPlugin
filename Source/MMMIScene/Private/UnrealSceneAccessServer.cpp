...
{
UnrealSceneAccessServer::UnrealSceneAccessServer( std::string sessionID,
                                                  UnrealSceneAccess* _SceneAccess )
    : SceneAccess( _SceneAccess ), sessionID( sessionID ),
...
void UnrealSceneAccessServer::InitializeServers()
{
    auto accessLambda = [this]() {
        this->remoteSceneAccessServer = new RemoteSceneAccessServer{ this->sessionID, shared_from_this() };
...
void UnrealSceneAccessServer::TerminationThriftClient()
{
    shared_ptr<TTransport> socket(
        new TSocket( MMISettings::GetInstance()->MIPRemoteSceneAccessAddress.Address,
