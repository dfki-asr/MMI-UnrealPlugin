...
UnrealSceneAccess::UnrealSceneAccess()
    : FrameID( 0 ), HistoryBufferSize( 20 ), sessionID( MMUAccess::GetNewGuid()) // this is completely broken. TODO:
{
}
...
void UnrealSceneAccess::InitializeServers()
{
    this->UESceneAccessServerCore = shared_ptr<SceneAccessServer::UnrealSceneAccessServer>(
        new SceneAccessServer::UnrealSceneAccessServer( sessionID, this ) );
    this->UESceneAccessServerCore->InitializeServers();

    this->started = true;
}

bool UnrealSceneAccess::IsStarted() {
    return this->started;
}
...
MBoolResponse UnrealSceneAccess::UpdateSceneObjects( vector<MSceneObjectUpdate> sceneObjects )
{
    ...
    {
		auto it = MMISceneObjectsByID.find(sceneObjectUpdate.ID);
        if(it != this->MMISceneObjectsByID.end())
        {
			UMMISceneObject* sceneObj = it->second;
			...
			sceneObj->MSceneObject.Transform.Position =  mVect3;
            // convert rotation to quarternion
            MQuaternion mQuat;
            MQuaternionExtensions::ToMQuaternion( mQuat, transformUpdate.Rotation );
			sceneObj->MSceneObject.Transform.Rotation = mQuat;
            // update the remaining fields
			sceneObj->MSceneObject.Transform.Parent = transformUpdate.Parent;
			sceneObj->MSceneObject.Mesh = sceneObjectUpdate.Mesh;
			sceneObj->MSceneObject.Collider = sceneObjectUpdate.Collider;
			sceneObj->MSceneObject.PhysicsProperties = sceneObjectUpdate.PhysicsProperties;
        }
        else
        {
...
{
	auto it = MMISceneObjectsByID.find(id);
    if(it != MMISceneObjectsByID.end())
        _return = it->second->MSceneObject.Collider;
}
...
void UnrealSceneAccess::GetMeshByID( MMesh& _return, const string& id )
{
	auto it = MMISceneObjectsByID.find(id);
    if(it != MMISceneObjectsByID.end() )
        _return = it->second->MSceneObject.Mesh;
}
...
void UnrealSceneAccess::GetTransformByID( MTransform& _return, const string& id )
{
	auto it = MMISceneObjectsByID.find(id);
	if (it != MMISceneObjectsByID.end())
		_return = it->second->MSceneObject.Transform;
}
